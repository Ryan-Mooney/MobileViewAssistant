...
4. Add this path to the name ("C:\MobileViewAssistant\IEDriverSoftware.exe") and add it to your computers PATH (do this as admin and regular user)
	...
	f.Add the drivers path plus its name(ie, "C:\MobileViewAssistant\IEDriverSoftware.exe")
...
6. Click Security.
7. Make sure that for each type of site, Protected Mode is either enabled or disabled for each (ie, all are enabled, or all are disabled). This is a settings that the Driver needs to be set to run properly.
8.Done! Exit out of admin and log back in as a normal user.

Opening the Software:
1. Go to the C:\MobileViewAssistant main folder.
...
2. Once open, enter your REGULAR username and password that you use to log in to Mobile View. 
...
1. Go to the folder with the files in it after installing pyinstaller
2. Run: pyinstaller --paths C:\Windows\WinSxS\x86_microsoft-windows-m..namespace-downlevel_31bf3856ad364e35_10.0.17134.1_none_50c6cb8431e7428f --hidden-import tkinter MVAssistantPopupDialog.py